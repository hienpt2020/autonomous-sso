...
export class ParserImpl implements IParser {
  ...
  parseWorkLayout(responseData: any): WorkLayout {
    ...
    const address = parseMapAddress(responseData.street, responseData.city, responseData.state, responseData.country);
    const image =
  ...
  parseWorkPlace(responseData: any): WorkPlace {
    ...
    result.devices = responseData.assets_details
      ...
      : [];
    result.tags = responseData.working_place_tags ? responseData.working_place_tags?.map((tag: any) => tag.tag) : [];
